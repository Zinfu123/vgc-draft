...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="grid grid-cols-1 items-center justify-center gap-6 lg:grid-cols-3">
            <!-- Current Picker - Larger (2 columns) -->
            <div
                class="overflow-hidden rounded-lg bg-white shadow-sm lg:col-span-2 dark:bg-gray-800/50 dark:shadow-none dark:outline dark:-outline-offset-1 dark:outline-white/10"
            >
                <div class="flex flex-col items-center justify-center px-6 py-8">
                    <h1 class="mb-6 text-2xl font-semibold">Current Picker</h1>
                    <img
            ...
            <div
                v-if="props.lastPick !== null && props.lastPick.team !== null"
                class="overflow-hidden rounded-lg bg-white shadow-sm lg:col-span-1 dark:bg-gray-800/50 dark:shadow-none dark:outline dark:-outline-offset-1 dark:outline-white/10"
            >
                <div class="px-4 py-4">
                    <h1 class="mb-3 text-center text-lg font-semibold">Last Pick</h1>
                    <div class="flex flex-row items-center justify-center gap-3">
                        ...
                        <div class="flex flex-shrink-0 flex-col items-center justify-center">
                            ...
                            />
                            <p class="text-md mt-1 text-center font-medium">Name: {{ props.lastPick.team.name }}</p>
                        </div>
                        <!-- Pokemon Card -->
            ...
            <ScrollArea class="h-[600px] w-full">
                <div class="flex flex-col items-center">
                    ...
                    >
                        ...
                        <ul role="list" class="grid grid-cols-1 justify-items-center gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
                            <li
                                v-for="draftOrderItem in props.draftOrders"
                                :key="draftOrderItem.id"
                                class="flex w-full max-w-[200px] flex-col divide-y divide-gray-200 rounded-lg bg-white text-center shadow-sm dark:divide-white/10 dark:bg-gray-800/50 dark:shadow-none dark:outline dark:-outline-offset-1 dark:outline-white/10"
                            >
                                <div class="flex flex-col p-4 sm:p-6">
                                    <img
                                        v-if="draftOrderItem.team && draftOrderItem.team.logo !== null"
                                        class="mx-auto size-20 shrink-0 rounded-full bg-gray-300 outline -outline-offset-1 outline-black/5 sm:size-24 md:size-28 dark:bg-gray-700 dark:outline-white/10"
                    ...
                    <div class="mb-6 flex flex-col gap-4 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-gray-800/50">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-end">
                            <div class="flex-1">
                                ...
                                <Input id="min-cost" type="number" v-model="minCost" placeholder="Min" min="0" />
                            </div>
                            <div class="flex-1">
                                <Label for="max-cost">Maximum Cost</Label>
                                <Input id="max-cost" type="number" v-model="maxCost" placeholder="Max" min="0" />
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-col gap-6">
                        <div v-for="costHeader in filteredCostHeaders" :key="costHeader" class="flex flex-col gap-4">
                            <div
                                class="sticky top-0 z-10 rounded-md border border-gray-200 bg-gray-100 px-4 py-3 text-center text-lg font-semibold text-gray-900 dark:border-white/10 dark:bg-gray-800 dark:text-white"
                            >
                                Cost: {{ costHeader }}
                            </div>
        ...
        <Dialog v-model:open="isDialogOpen">
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription> Are you sure you want to draft this Pokemon? </DialogDescription>
                </DialogHeader>
