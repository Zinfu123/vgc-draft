...
const STORAGE_KEY = 'vgc-team-coverage-planner-v2';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Team coverage', href: '/v2/team-coverage' },
];
...
async function runSearch(side: 'ally' | 'enemy', index: number): Promise<void> {
    ...
    const url = route('v2.team-coverage.pokedex-search') + '?' + params.toString();
...
async function loadLearnset(side: 'ally' | 'enemy', index: number): Promise<void> {
    ...
    try {
        const url = route('v2.team-coverage.learnset', { pokedex: slot.pokedexId }) + '?game=' + encodeURIComponent(selectedVersionSlug.value);
        const res = await jsonFetch(url);
        if (!res.ok) {
            return;
        }
...
async function loadRoster(): Promise<void> {
    ...
    try {
        const url = route('v2.team-coverage.roster', { team: importTeamId.value });
        const res = await jsonFetch(url);
        if (!res.ok) {
            return;
        }
