...
class PokedexPagesService implements PokedexPages
{
    ...
    public function indexProps(array $validated): array
    {
        ...
        $game = isset($validated['game']) ? trim((string) $validated['game']) : '';
        $ability = isset($validated['ability']) ? trim((string) $validated['ability']) : '';
        $move = isset($validated['move']) ? trim((string) $validated['move']) : '';

        if ($game === '') {
            $game = (string) config('pokemon.default_version_group_slug');
        }
        ...
        $versionGroupId = $versionGroups->firstWhere('slug', $game)?->id
            ?? $versionGroups->first()?->id;
        ...
        return [
            ...
            'abilityFilterOptions' => $versionGroupId !== null
                ? PokedexFilterService::abilityFilterOptionsForVersionGroup((int) $versionGroupId)
                : [],
        ];
    }
