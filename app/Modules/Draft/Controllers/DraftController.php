...
class DraftController extends Controller
{
    ...
    public function pick(Request $request, DraftPokemonAction $draftPokemonAction, ReadLeagueDraftAction $readLeagueDraftAction)
    {
        ...
        $matchConfig = MatchConfig::where('league_id', $leagueId)->select('minimum_drafts')->first();
        
        if (!$matchConfig) {
            throw new \Exception('MatchConfig not found for league ' . $leagueId);
        }
        
        if (!$draft) {
            throw new \Exception('Draft not found for league ' . $leagueId);
        }
        
        $mandatoryPicks = $matchConfig->minimum_drafts - $draft->round_number;
