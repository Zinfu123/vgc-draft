<?php

namespace App\Modules\Draft\Actions;

use App\Models\User;
use App\Modules\Draft\Models\BanOrder;
use App\Modules\Draft\Models\Draft;
use App\Modules\Draft\Models\DraftOrder;
use App\Modules\League\Models\League;
use App\Notifications\DraftTurnReminderNotification;

class RemindCurrentPickerAction
{
    /**
     * @param  array{league_id:int, remaining_seconds:int}  $data
     */
    public function __invoke(array $data): void
    {
        $leagueId = (int) $data['league_id'];
        $remainingSeconds = (int) $data['remaining_seconds'];

        if ($remainingSeconds <= 0) {
            return;
        }

        $league = League::query()->find($leagueId);
        if (! $league || ! $league->discord_webhook_url) {
            return;
        }

        $draft = Draft::query()->where('league_id', $leagueId)->first();
        if (! $draft || (int) $draft->status === 0) {
            return;
        }

        $status = (int) $draft->status;

        if ($status === 2) {
            $current = BanOrder::query()
                ->where('league_id', $leagueId)
                ->where('status', 1)
                ->orderBy('round_number')
                ->orderBy('ban_number')
                ->first();
            $phase = 'ban';
        } else {
            $current = DraftOrder::query()
                ->where('league_id', $leagueId)
                ->where('status', 1)
                ->orderBy('pick_number')
                ->first();
            $phase = 'pick';
        }

        if (! $current) {
            return;
        }

        $userId = $current->user_id;
        if (! $userId) {
            $current->loadMissing('team');
            $userId = $current->team?->user_id;
        }

        if (! $userId) {
            return;
        }

        $user = User::query()->find($userId);
        if (! $user) {
            return;
        }

        $league->notify(new DraftTurnReminderNotification($league, $user, $phase, $remainingSeconds));
    }
}
